    ...
    if (newStatus && currentTask.status !== newStatus) {
      ...
      try {
        ...
        await logActivity(
          activeId,
          'status_change',
          `moved task from ${oldStatus.replace('-', ' ')} to ${newStatus.replace('-', ' ')}`,
          { uid: user!.uid, displayName: user!.displayName, photoURL: user!.photoURL }
        );

        // Notify Assignees & Creator
        const recipients = new Set<string>();
        if (currentTask.createdBy && currentTask.createdBy !== user!.uid) recipients.add(currentTask.createdBy);
        currentTask.assignees?.forEach(a => {
          if (a.uid !== user!.uid) recipients.add(a.uid);
        });

        const notifyPromises = Array.from(recipients).map(uid =>
