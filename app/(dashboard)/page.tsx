...
interface Notification {
    id: string;
    type: 'task_assigned' | 'comment' | 'status_change' | 'reply';
    senderName: string;
    senderPhoto?: string;
    taskTitle: string;
        ...
        async function fetchDashboardData() {
            ...
            try {
                ...
                const sortedTasks = [...allTasks]
                    .filter(task => { // Add Privacy Filter
                        if (task.visibility === 'private') {
                            const isCreator = task.createdBy === user.uid;
                            const isAssignee = task.assignees?.some(a => a.uid === user.uid);
                                        ...
                                        <div key={notif.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors flex gap-3 items-start relative group">
                                            ...
                                            <div className="flex-1 space-y-1 min-w-0">
                                                <p className="text-sm text-slate-800 dark:text-slate-200 break-words leading-snug">
                                                    <span className="font-semibold">{notif.senderName || "Unknown User"}</span>
                                                    {notif.type === 'task_assigned' && <span className="text-slate-500"> assigned you a task: </span>}
                                                    {notif.type === 'comment' && <span className="text-slate-500"> commented on: </span>}
                                                    {notif.type === 'status_change' && <span className="text-slate-500"> updated status of: </span>}
                                                    {notif.type === 'reply' && <span className="text-slate-500"> replied to you on: </span>}

                                                    <span className="font-medium text-primary block truncate">{notif.taskTitle || "Untitled Task"}</span>
                                                </p>
                                            ...
                                            <div className="flex-shrink-0 pt-1">
                                                {notif.type === 'task_assigned' && <CheckSquare className="h-4 w-4 text-blue-500" />}
                                                {notif.type === 'comment' && <MessageSquare className="h-4 w-4 text-green-500" />}
                                                {notif.type === 'status_change' && <CheckCircle2 className="h-4 w-4 text-orange-500" />}
                                                {notif.type === 'reply' && <MessageSquare className="h-4 w-4 text-purple-500" />}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            recentTasks.map(task => (
                                <Card key={task.id} className="hover:shadow-md transition-all overflow-hidden group border-l-4 border-l-transparent hover:border-l-primary">
                                    <div className="flex items-center p-4 gap-4">
                                        <div className={`h-2 w-2 rounded-full flex-shrink-0 ${task.priority === 'high' ? 'bg-red-500' :
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-muted-foreground truncate">{task.description?.replace(/<[^>]*>/g, '') || "No description"}</p>
                                        </div>
